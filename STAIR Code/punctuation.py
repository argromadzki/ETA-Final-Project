        ...
        for counter, article in enumerate(csv.reader(inf)):
            if header:
                ...
                if stripspecial:
                    ...
                    try:
                        article_text = unidecode(article_text.decode('utf-8-sig'))
                    except:
                        try:
                ...
                # if id % 1000 == 1:
                #     print "\n*** %d ***\n" % id
                #     print [article[col] for col in keepcols] + [preppedtext[:50],]
...
def punctuationPreprocess(aText, lang='english'):
    ...
    # 2a. Arab words with internal ' (mostly handled in translation dictionary)
    ...
    # B. internal breaks become commas
...
def punctuation_english(aText):
    ...
    aText = re.sub("\\bpandora's box\\b", "pandoras box", aText,
                   flags=re.IGNORECASE)
    # Remove general possessive 's (very common, no useful info) - but maybe should count as a word?!
    aText = re.sub("\\b([a-z]+)'s\\b", "\\1", aText,
                   flags=re.IGNORECASE)
...
def punctuation_german(aText):
    ...
    # Removed umlauts on halfte, funftel
    aText = fraction_to_word(aText,
                             (' die halfte ', ' ein drittel ', ' zwei drittel ',
                              ' ein viertel ', ' drei viertel ', ' ein funftel '))
    aText = char_to_word(aText,
